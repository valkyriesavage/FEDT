...
def random_param_set():
    ...
    return (random.choice(list(arange(5,90,5)) + list(arange(95,180,5))), # bottom angle
            ...
            random.choice(arange(.15, 1.00, .05))) # infill density / width

@fedt_experiment
def cross_validation():
    large = [VolumeFile("gid_mug.stl") for i in range(10)]
    medium = [VolumeFile("gid_keysleeve.stl") for i in range(6)]
    objs = large + medium
